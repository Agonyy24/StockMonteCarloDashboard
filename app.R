...
ui <- fluidPage(
  theme = bs_theme(
    bootswatch = "darkly",  # np. cerulean, cosmo, darkly, united
    base_font = "Roboto"
  ),
  ...
  navbarPage(
    ...
    tabPanel("Polish Market Simulation",
             h3("📈 Simulation for Polish Stocks (Stooq) — Coming Soon"),
             p("This section will include Monte Carlo simulations for Polish companies 
                based on historical data from stooq.pl.")
    ),
    ...
    # --- Tab 5: Student-t Monte Carlo ---
    tabPanel("Student-t Simulation",
             h3("🎲 Monte Carlo with Student-t Distribution"),
             p("Future feature: simulate asset prices assuming a fat-tailed Student-t distribution 
                to better capture market extremes and volatility clustering.")
    )
  )
)
...
server <- function(input, output) {
  ...
  observeEvent(input$run, {
    ...
    output$mcPlot <- renderPlot({
      matplot(sim, type = "l", lty = 1, col = rgb(0, 0, 1, 0.15),
    ...
    output$histPlot <- renderPlot({
      hist(returns, breaks = 40, col = "lightblue", border = "white",
